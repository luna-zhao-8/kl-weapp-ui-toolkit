...
async function checkIncludedComponents(jsonPath, componentListMap) {
  ...
  // 进入存储
  const extListMap = {
    wxml: componentListMap.wxmlFileList,
    wxss: componentListMap.wxssFileList,
    json: componentListMap.jsonFileList,
    js: componentListMap.jsFileList,
    ts: componentListMap.jsFileList,
  }
  Object.keys(extListMap).forEach(async (ext) => {
    const isExist = await _.checkFileExists(path.join(dirPath, `${fileName}.${ext}`))
    if (isExist) {
      extListMap[ext].push(`${fileBase}.${ext}`)

      if (ext === 'js' || ext === 'ts') {
        componentListMap.jsFileMap[fileBase] = `${path.join(dirPath, fileName)}.${ext}`
      }
    }
  })
}
